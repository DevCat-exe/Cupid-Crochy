...
import { CreditCard, Calendar, DollarSign, CheckCircle2, XCircle, Clock, ArrowLeft } from "lucide-react";
...
export default function UserPaymentsPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brand-beige/20 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-brand-beige/20 pt-32 pb-16">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8 flex items-center gap-4">
            <Link
              href="/account"
              className="p-2 rounded-xl hover:bg-white text-brand-maroon/40 hover:text-brand-maroon transition-all"
            >
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <div>
              <h1 className="text-4xl font-bold text-brand-maroon mb-2">Payment History</h1>
              <p className="text-brand-maroon/60">Track all your transactions and payments</p>
            </div>
          </div>
          ...
          {payments.length === 0 ? (
            <div className="bg-white rounded-[2.5rem] p-16 text-center border border-brand-maroon/5">
              ...
              <Link
                href="/products"
                className="inline-flex items-center gap-2 bg-brand-maroon text-white font-bold px-8 py-4 rounded-2xl hover:bg-brand-maroon/90 transition-all"
              >
                Start Shopping
              </Link>
            </div>
